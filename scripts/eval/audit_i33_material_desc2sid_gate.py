#!/usr/bin/env python3
"""Evaluate I-33 combined r104 candidates on frozen E-clean gates."""
...
HOLDOUT_SHA = "76acc6a39b248e4501a11e99bc889871f5208c811070485791b554538e658f99"
WORLD_HOLDOUT_SHA = "f75106758792163dd33d1d52639ba507a6d9e69094d8213d5f3b0969ee272f62"
...
RETENTION_TASKS = ("action", "topic", "rec_video", "rec_prod", "rec_ad", "rec_living")
EXPECTED_COUNTS = {
    "material_desc2sid": 256,
...
def render(row: dict[str, Any]) -> tuple[str, str]:
    if row.get("history"):
        raise RuntimeError("I-33 gate rows unexpectedly contain history")
...
def response_body(ids: list[int], material: bool) -> tuple[int, int]:
    ...
    if start >= end:
        raise RuntimeError("empty I-33 gate response body")
    return start, end
...
def sampled_relative_positions(start: int, end: int, cap: int):
    ...
    if values.numel() != cap:
        raise RuntimeError("I-33 gate cap produced duplicate positions")
    return values + start
...
def forward_kl(policy, reference) -> float:
    ...
    if not math.isfinite(value):
        raise RuntimeError("non-finite I-33 gate KL")
    return value
...
def mean_gold_logp(logits, targets) -> float:
    ...
    if not math.isfinite(value):
        raise RuntimeError("non-finite I-33 gate gold logp")
    return value
...
def extract_answer(output: str) -> tuple[str, int, int]:
    ...
    if not matches:
        raise RuntimeError(f"unparsed I-33 world answer: {output[-160:]!r}")
...
def main() -> None:
    ...
    required = {
        ...
        HOLDOUT: HOLDOUT_SHA,
        WORLD_HOLDOUT: WORLD_HOLDOUT_SHA,
    }
    for path, expected in required.items():
        if not path.is_file() or sha256(path) != expected:
            raise RuntimeError(f"locked I-33 gate artifact missing or drifted: {path}")
    for name, path in args.candidate:
        ...
        if (int(config["r"]), int(config["lora_alpha"])) != (104, 104):
            raise RuntimeError(f"{name} is not exact r104/alpha104")

    rows = [json.loads(line) for line in HOLDOUT.open() if line.strip()]
    rows.extend(
        row
        for row in (json.loads(line) for line in WORLD_HOLDOUT.open() if line.strip())
        if row["task"] == "world"
    )
    if len(rows) != 736 or Counter(row["task"] for row in rows) != EXPECTED_COUNTS:
        raise RuntimeError("I-33 holdout row/task contract drifted")
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if row["task"] == "world":
                ...
                if row["output"][answer_start:answer_end].upper() != answer:
                    raise RuntimeError("I-33 world answer extraction span drifted")
                ...
                if not target_indexes or target_indexes != list(
                    ...
                    raise RuntimeError("I-33 world answer token span is empty or disjoint")
                if target_indexes[0] == 0:
                    raise RuntimeError("I-33 world answer starts at token zero")
            ...
            if index % 32 == 0 or index == len(rows):
                print(f"[i33-gate] {index}/{len(rows)} elapsed={time.time()-started:.1f}s", flush=True)
            del input_ids, attention_mask, logits, positions, targets

    reports = {}
    parent_world = world_exact["parent"]["correct"]
    for name, path in args.candidate:
        desc2sid = summarize_material(by_candidate[name]["material_desc2sid"])
        sid2desc = summarize_retention(by_candidate[name]["material_sid2desc"])
        sid2desc["pass"] = (
            sid2desc["top1_agreement"] >= 0.99
            and sid2desc["parent_to_candidate_kl_mean"] <= 0.005
            and sid2desc["gold_mean_logp_delta"] >= -0.01
        )
        ...
        world_pass = world_exact[name]["rows"] == 16 and world_exact[name]["correct"] >= parent_world
        reports[name] = {
            "path": str(path.resolve()),
            "adapter_sha256": sha256(path / "adapter_model.safetensors"),
            "material_desc2sid": desc2sid,
            "material_sid2desc_protection": sid2desc,
            "retention": retention,
            "world_exact": {
            ...
            "teacher_forced_pass": desc2sid["pass"] and sid2desc["pass"] and retention_pass and world_pass,
        }
    ...
    report = {
        ...
        "holdout": {
            ...
            "rows_without_world": 720,
            "world_path": str(WORLD_HOLDOUT.resolve()),
            "world_sha256": sha256(WORLD_HOLDOUT),
            "world_rows": 16,
            "total_rows": len(rows),
        },
