#!/usr/bin/env python3
"""Evaluate I-31 exact r96/I-23 interpolation candidates on the development set."""
...
HOLDOUT_SHA = "dd744ee2d2f584b9bcae938cde1f5976801a9eece39aa1972b284641603f97a0"
...
EXPECTED_COUNTS = {**{task: 128 for task in MATERIAL_TASKS}, **{task: 64 for task in RETENTION_TASKS}}
IM_END = "<|im_end|>"
...
def render(row: dict[str, Any]) -> tuple[str, str]:
    if row.get("history"):
        raise RuntimeError("I-30 gate rows unexpectedly contain history")
...
def response_body(ids: list[int], material: bool) -> tuple[int, int]:
    ...
    if start >= end:
        raise RuntimeError("empty I-30 gate response body")
    return start, end
...
def sampled_relative_positions(start: int, end: int, cap: int):
    ...
    if values.numel() != cap:
        raise RuntimeError("I-30 gate cap produced duplicate positions")
    return values + start
...
def forward_kl(policy, reference) -> float:
    ...
    if not math.isfinite(value):
        raise RuntimeError("non-finite I-30 gate KL")
    return value
...
def mean_gold_logp(logits, targets) -> float:
    ...
    if not math.isfinite(value):
        raise RuntimeError("non-finite I-30 gate gold logp")
    return value
...
def extract_answer(output: str) -> tuple[str, int, int]:
    ...
    if not matches:
        raise RuntimeError(f"unparsed I-30 world answer: {output[-160:]!r}")
...
def main() -> None:
    ...
    for path, expected in required.items():
        if not path.is_file() or sha256(path) != expected:
            raise RuntimeError(f"locked I-30 gate artifact missing or drifted: {path}")
    for name, path in args.candidate:
        ...
        if (int(config["r"]), int(config["lora_alpha"])) != (160, 160):
            raise RuntimeError(f"{name} is not exact r160/alpha160")

    rows = [json.loads(line) for line in HOLDOUT.open() if line.strip()]
    if len(rows) != 704 or Counter(row["task"] for row in rows) != EXPECTED_COUNTS:
        raise RuntimeError("I-30 holdout row/task contract drifted")
        ...
        for index, row in enumerate(rows, start=1):
            ...
            if row["task"] == "world":
                ...
                if row["output"][answer_start:answer_end].upper() != answer:
                    raise RuntimeError("I-30 world answer extraction span drifted")
                ...
                if not target_indexes or target_indexes != list(
                    ...
                    raise RuntimeError("I-30 world answer token span is empty or disjoint")
                if target_indexes[0] == 0:
                    raise RuntimeError("I-30 world answer starts at token zero")
            ...
            if index % 32 == 0 or index == len(rows):
                print(f"[i30-gate] {index}/{len(rows)} elapsed={time.time()-started:.1f}s", flush=True)
    ...
    for name, path in args.candidate:
        ...
        world_pass = world_exact[name]["rows"] == 64 and world_exact[name]["correct"] >= parent_world
        reports[name] = {
