...
if [[ $# -ne 3 ]]; then
  echo "usage: WANDB_ENTITY=<entity> WANDB_PROJECT=<project> $0 <single-gpu-id-or-uuid> <configs/active/i23_action_ansretkl_v1.yaml> <logs/train/run.log>" >&2
  exit 2
fi
...
[[ "$config" == "$root/configs/active/i23_action_ansretkl_v1.yaml" ]] || {
  echo "I-24 detached launcher received the wrong config: $config" >&2
  exit 2
}
...
nohup setsid --wait bash -c '
  ...
  printf "%s\n" "$rc" >"$status"
  exit "$rc"
' bash "$root/scripts/train/launch_i24_wandb_online.sh" "$gpu_id" "$config" "$status_path" \
