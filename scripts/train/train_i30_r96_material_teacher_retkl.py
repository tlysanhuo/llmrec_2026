...
MATERIAL_TEACHER_KL = float(os.environ.get("I30_MATERIAL_TEACHER_KL", "0.5"))
MATERIAL_PARENT_KL = float(os.environ.get("I30_MATERIAL_PARENT_KL", "0.1"))
RETENTION_PARENT_KL = float(os.environ.get("I30_RETENTION_PARENT_KL", "4.0"))
RETENTION_MAX_POSITIONS = int(os.environ.get("I30_RETENTION_MAX_POSITIONS", "96"))
LOGIT_CHUNK = int(os.environ.get("I30_LOGIT_CHUNK", "8"))
...
def target_span(labels: torch.Tensor) -> tuple[int, int]:
    if labels.ndim != 2 or labels.size(0) != 1:
        raise RuntimeError("I-30 requires per-device batch size 1")
    positions = torch.nonzero(labels[0].ne(IGNORE_INDEX), as_tuple=False).flatten()
    if positions.numel() == 0:
        raise RuntimeError("I-30 batch has no response targets")
    start, end = int(positions[0]), int(positions[-1]) + 1
    if start == 0:
        raise RuntimeError("I-30 response starts at token zero")
    expected = torch.arange(start, end, device=positions.device)
    if not torch.equal(positions, expected):
        raise RuntimeError("I-30 forbids packing and disjoint response spans")
    return start, end
...
def body_bounds(tokens: list[int]) -> tuple[int, int] | None:
    ...
    if start >= end or tokens[end - 1] != EOS_ID:
        raise RuntimeError("I-30 response body is empty or missing EOS")
    return start, end
...
def route_response(targets: torch.Tensor, prompt_tokens: list[int]) -> tuple[str, int, int]:
    ...
    if bounds is None:
        ...
        if end <= 0:
            raise RuntimeError("I-30 legacy retention response is empty")
...
def capped_positions(start: int, end: int, cap: int, device: torch.device) -> torch.Tensor:
    count = end - start
    if count <= 0:
        raise RuntimeError("I-30 cannot select positions from an empty body")
    if count <= cap:
    ...
    if relative.numel() != cap:
        raise RuntimeError("I-30 retention position cap produced duplicates")
    return relative + start


def token_ce(logits: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
    if logits.shape[:2] != (1, targets.numel()):
        raise RuntimeError(f"I-30 CE shape mismatch: {tuple(logits.shape)}/{targets.shape}")
...
def forward_kl(policy_logits: torch.Tensor, reference_logits: torch.Tensor) -> torch.Tensor:
    if policy_logits.shape != reference_logits.shape:
        raise RuntimeError(f"I-30 KL shape mismatch: {policy_logits.shape}/{reference_logits.shape}")
...
def single_adapter_name(value: object) -> str:
    ...
    if isinstance(value, (list, tuple)) and len(value) == 1 and isinstance(value[0], str):
        return value[0]
    raise RuntimeError(f"I-30 expected one active residual adapter, got {value!r}")


def ensure_models(trainer: Any, model: Any):
    state = getattr(trainer, "_i30_state", None)
    ...
    if observed != EXPECTED_TRAINER:
        raise RuntimeError(f"I-30 trainer contract drifted: {observed}/{EXPECTED_TRAINER}")
    report_to = args.report_to if isinstance(args.report_to, list) else [args.report_to]
    if "wandb" not in report_to:
        raise RuntimeError(f"I-30 requires W&B online reporting: {report_to!r}")

    peft_config = getattr(unwrapped, "peft_config", None)
    if not isinstance(peft_config, dict) or len(peft_config) != 1:
        raise RuntimeError("I-30 expected exactly one fresh residual after merging r96")
    ...
    if rank_alpha != EXPECTED_RANK_ALPHA:
        raise RuntimeError(f"I-30 expected r8/alpha8, got {rank_alpha}")
    ...
    if not trainable or any("lora_" not in name for name in trainable):
        raise RuntimeError("I-30 trainable set is empty or contains non-LoRA parameters")
    if sha256(TEACHER_ADAPTER / "adapter_model.safetensors") != TEACHER_SHA256:
        raise RuntimeError("I-30 frozen I-23 teacher hash drifted")
    ...
    trainer._i30_state = state
    print(
        f"[i30] contract PASS: merged-r96 parent + fresh r8/alpha8; "
...
def paired_parent_policy(trainer, model, inputs, positions):
    ...
    if not state["fingerprint_checked"]:
        ...
        if max_abs > 1e-4:
            raise RuntimeError(f"I-30 step-0 r96 fingerprint failed: {max_abs:.8f}")
        state["fingerprint_checked"] = True
        print(f"[i30] step-0 r96 fingerprint PASS: max_abs={max_abs:.8f}", flush=True)
    return outputs, parent, state


def record_route(route: str) -> tuple[int, Counter[str]]:
    count = getattr(i30_loss, "call_count", 0) + 1
    counts = Counter(getattr(i30_loss, "route_counts", Counter()))
    counts[route] += 1
    if count > EXPECTED_ROWS or counts[route] > EXPECTED_ROUTES[route]:
        raise RuntimeError(f"I-30 route count exceeded contract: {count}/{dict(counts)}")
    remaining = EXPECTED_ROWS - count
    for name, expected in EXPECTED_ROUTES.items():
        if counts[name] + remaining < expected:
            raise RuntimeError(f"I-30 remaining rows cannot satisfy {name}: {dict(counts)}")
    if count == EXPECTED_ROWS and dict(counts) != EXPECTED_ROUTES:
        raise RuntimeError(f"I-30 final route mismatch: {dict(counts)}/{EXPECTED_ROUTES}")
    i30_loss.call_count = count
    i30_loss.route_counts = counts
    return count, counts


def i30_loss(trainer, model, inputs, return_outputs=False, **kwargs):
    ...
    if count <= 8 or count % 128 == 0 or count == EXPECTED_ROWS:
        print(
            f"[i30] microbatch={count}/{EXPECTED_ROWS} route={route} "
...
def run_data_preflight() -> None:
    ...
    with TRAINING_DATA.open(encoding="utf-8") as handle:
        for line_number, line in enumerate(handle, start=1):
            ...
            if route != expected:
                raise RuntimeError(
                    f"I-30 preflight route mismatch at line {line_number}: "
                    f"{route}/{expected} task={row['task']}"
                )
            if body_end <= body_start:
                raise RuntimeError(f"I-30 empty scored body at line {line_number}")
            length = len(prompt_ids) + len(response_ids)
            if length > 16384:
                raise RuntimeError(f"I-30 cutoff overflow at line {line_number}: {length}")
            maximum = max(maximum, length)
            counts[route] += 1
            task_counts[row["task"]] += 1
    if dict(counts) != EXPECTED_ROUTES:
        raise RuntimeError(f"I-30 preflight route signature mismatch: {dict(counts)}")
    print(
        f"[i30] data preflight passed: rows={sum(counts.values())} "
...
def main() -> None:
    ...
    def patched(self, model, inputs, *args, **kwargs):
        ...
        return i30_loss(self, model, inputs, **kwargs)
