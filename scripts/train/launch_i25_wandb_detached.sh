...
if [[ $# -ne 3 ]]; then
  echo "usage: WANDB_ENTITY=<entity> WANDB_PROJECT=<project> $0 <single-gpu-id-or-uuid> <configs/active/i23_actionres_r16_ansretkl_ep1.yaml> <logs/train/run.log>" >&2
  exit 2
fi
...
[[ -n "$gpu_id" && "$gpu_id" != *,* ]] || {
  echo "I-25 detached training must expose exactly one GPU" >&2
  exit 2
}
[[ -x "$python_bin" ]] || { echo "python is not executable: $python_bin" >&2; exit 2; }
[[ -f "$config" ]] || { echo "missing config: $config" >&2; exit 2; }
config=$(realpath "$config")
[[ "$config" == "$root/configs/active/i23_actionres_r16_ansretkl_ep1.yaml" ]] || {
  echo "I-25 detached launcher received the wrong config: $config" >&2
  exit 2
}
[[ -f "$gate" ]] || { echo "missing I-25 preregistered gate: $gate" >&2; exit 2; }
[[ "$(sha256sum "$gate" | awk '{print $1}')" == "$expected_gate" ]] || {
  echo "I-25 preregistered gate checksum drifted" >&2
  exit 2
}
...
expected_paths = {
    "config": "configs/active/i23_actionres_r16_ansretkl_ep1.yaml",
...
for name, relative in expected_paths.items():
    if artifacts[name].get("path") != relative:
        raise SystemExit(f"I-25 gate {name} path drifted: {artifacts[name]}")
for name in ("config", "trainer"):
    record = artifacts[name]
    actual = hashlib.sha256((root / record["path"]).read_bytes()).hexdigest()
    if actual != record.get("sha256"):
        raise SystemExit(f"I-25 gate {name} hash mismatch: {actual}")
...
if drift:
    raise SystemExit(f"I-25 gate invariant drift: {drift}")
axis = gate["checkpoint_axis"]
if axis.get("output_root") != "checkpoints/i23_actionres_r16_ansretkl_ep1":
    raise SystemExit("I-25 gate output root drifted")
if axis.get("candidates_in_ascending_order") != [250, 500, 750, 1000, 1250, 1527]:
    raise SystemExit("I-25 gate candidate order drifted")
print("[i25] detached preregistered gate precheck PASS")
PY
[[ ! -e "$root/checkpoints/i23_actionres_r16_ansretkl_ep1" && ! -L "$root/checkpoints/i23_actionres_r16_ansretkl_ep1" ]] || {
  echo "refusing to overwrite I-25 output directory" >&2
  exit 2
}
...
nohup setsid --wait bash -c '
  ...
  printf "%s\n" "$rc" >"$status"
  exit "$rc"
' bash "$root/scripts/train/launch_i25_wandb_online.sh" "$gpu_id" "$config" "$status_path" \
