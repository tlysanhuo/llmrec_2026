...
> 变更记录（2026-07-23 UTC）：I-40当前有效训练为GPU1独立后台W&B run [`34k0sdcj`](https://wandb.ai/thaongocnguyendo0-/llmrec-2026/runs/34k0sdcj)，后台wrapper为PPID1/独立session，不依赖交互终端；从原始I-35 step548 r112全新启动，真实reference/policy/optimizer/step0 logits门再次全部通过。前一run [`9dp9wnbo`](https://wandb.ai/thaongocnguyendo0-/llmrec-2026/runs/9dp9wnbo)本地到step169后无错误栈地被外部终止，W&B为`crashed`、没有step515 checkpoint，已完整归档且禁止resume。实验仍是直接续训同一r112，不训练fresh低秩LoRA、不参数拼接；正式混合为I-36审计用户5,500条与I-35原正式2,740条，共8,240次暴露。
...
- 当前P0为用户已授权的I-40：只从保留的I-35 step548 r112直接续训，使用I-36审计用户样本与I-35全量原训练行自KL回放；I-36/I-37失败checkpoint继续禁止resume。I-40不把I-37改scale或补step256，也不新增/拼接低秩adapter。
