...
- I-18已将官方源派生I-10父数据中的538条上游截断推荐CoT做历史证据约束语义修复；538/538通过程序门和独立judge score=5九项全真质检。新数据32,644行/SHA256 `634c4805...c0e4`，仅538条CoT变化，prompt/答案/行序/任务数/O2 teacher均不变，目标元数据/T/E为0，16k超限0。正式实验从O6复用I-10 r64/lr1e-4/3ep全部训练字段，不采用融合或失败checkpoint；2026-07-14 17:26 UTC在GPU1持久启动，1,995/1,995正常完成，train loss 1.3618，W&B [`32av8e8z`](https://wandb.ai/3120252125-/llmrec-2026/runs/32av8e8z)服务端`finished`。E3/root adapter SHA256 `07cd6628...2a9e3`且逐字节一致，itemic结构断裂0/60=`PASS`；E3严格两文件包已验收、尚未上传或消耗线上次数，本地门禁不代表已证明涨分。候选数据、manifest和恢复说明位于[`assets/derived/releases/seed_teacher_cotfix_v2/`](assets/derived/releases/seed_teacher_cotfix_v2/)；这是未评测候选发布，不是最高分实现发布。
