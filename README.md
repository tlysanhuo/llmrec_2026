...
- 当前最高分实现是I-13 `e3_userres_r80_retkl_v3_s875`，固定协议线上`0.9978`。它由I-10 r64父adapter与I-12 r16用户残差按`0.875`精确拼为r80，属于融合合规灰区；不是一次直接r80训练。两份完整训练数据、三阶段可移植配置、loss/拼接脚本、manifest和原始小型审计已发布到[`assets/derived/releases/e3_userres_r80_retkl_v3_s875/`](assets/derived/releases/e3_userres_r80_retkl_v3_s875/)，统一入口为`scripts/reproduce/i13_highscore.sh`。
- I-23 `seed_teacher_cotfix_v3_r64_lr1e4_ep3`固定协议E3线上`0.9915`，八项=`0.2760/0.1099/0.0383/0.0576/0.1258/0.1400/0.1053/0.1387`。它从O6直接训练单个r64 adapter，不做参数拼接，现为固定协议最高无参数拼接单adapter；仍含164条官方来源派生teacher行，不能写成纯O1。相对I-13总分低0.0063，但material高0.0307；已获准作为新action-answer-token CE + 冻结I-23 KL保持实验的父模型，I-13仍是当前最高榜分实现。
- score-first首投`I-23 + 0.625×I-12用户残差`已线上完成为`0.9866`，八项=`0.2453/0.1170/0.0399/0.0768/0.1326/0.1316/0.1044/0.1390`，未超过I-13。相对同日I-23复测`0.9884`，七个非material项净增`+0.0289`：用户`+0.0090`、video/prod `+0.0294`，但material掉档`-0.0307`，净结果`-0.0018`。这证明残差机制有效而0.625越过material临界点；下一发降到`s500`。若material保持，按本次七项响应线性缩放的条件中心为`1.0115`；若仍掉档则约`0.9808`，两者都不是校准保证。严格两文件包已就绪，尚未上传。
- I-25 `i23_actionres_r16_ansretkl_ep1`已完成1,527/1,527；step250缺失不影响实战检查。按原冻结协议六个scale1点均失败，因此字面结论仍是`ORIGINAL_GATE_FAIL`，但不能外推线上方向。当前1024复评中step500的scale `0.5/0.625/0.75`分别为JSON `26/28/29`、截断`6/4/3`，scale0.75结构最接近I-23；material与推荐保护诊断没有发现硬灾难，但也不能证明上分。该分支优先级低于保留完整I-23的`s625`跨父残差首投；不打包、不上传，I-26重训仍未获授权。
...
- 固定协议结果为I-11/I-12/I-13/I-14/I-17/I-18/I-19/I-23=`0.9618/0.9768/0.9978/0.9518/0.9727/0.9697/0.9763/0.9915`。I-13相对I-12总分`+0.0210`，action/topic=`-0.0023/-0.0003`，video/prod/ad/live=`+0.0288/-0.0068/0/+0.0009`，world=`+0.0007`；当前固定协议主模型仍为I-13。I-23原始日志也已复核为action1024+itemic 7次race-average、8/8完成且失败0。
