...
**评测配额**:用户于2026-07-14确认当前每日 **5 次**；**北京时间 15:00 刷新**、**账号级与队友共享**，失败不消耗。评测耗时 ~1h10-1h35。**评测方差警告(07-04 选手情报):同一模型可打出 0.86-0.94**——这只证明非确定性风险，不能据单组复测标定通用±0.04区间；比较仍优先看同协议直接对照与分项结构。
...
- **LoRA上传大小实测限制（2026-07-20）**：平台在任务创建前硬校验上传文件夹总大小不超过400MB；423,941,100-byte两文件包被明确拒绝，无evalTaskId。高rank拼接adapter必须在上传前检查两文件总字节数，必要时用BF16存储并重新完成门控与结构体检，禁止靠删张量或改rank规避。
- **LoRA rank实测限制（2026-07-20）**：平台要求`r`为`1~128`整数；总大小已降到211,997,868 bytes的r168/alpha168包仍在任务创建前被明确拒绝。上传前必须同时核验`1<=r<=128`与总大小小于400,000,000 bytes；降rank必须对真实权重做可复核的低秩变换并重新跑门控，不能只改`adapter_config.json`。
...
以后交付任何候选包时，必须同时给出：SCP命令、包路径、两文件SHA256、两文件总大小（必须低于400MB）、LoRA rank（必须为1~128整数）和上述表单逐项值。若同一实验族后续上传另一个checkpoint，才选择“已有模型新增版本”；不同实验族仍选择“新建模型”。
