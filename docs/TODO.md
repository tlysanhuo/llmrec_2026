...
- [x] 完成I-23 E3固定协议评测：线上0.9915，八项=`0.2760/0.1099/0.0383/0.0576/0.1258/0.1400/0.1053/0.1387`；action1024、itemic 7次race-average、8/8完成、失败0。它未替换I-13 0.9978，但成为固定协议最高无参数拼接单adapter；用户已批准只将成功的I-23 E3作为新action-answer-token CE + 冻结I-23 KL保持实验的父模型，E1/E2仍只作剂量轨迹。
...
- [ ] 提交material临界点下一探针`I-23 + 0.5×I-12用户残差`：严格两文件包`submissions/i23_userres_r80_s500_platform/`已就绪，adapter/config SHA256=`d2b77c74...99bf`/`4768770a...4d06`。按s625相对同日I-23的七项响应线性缩放，material保持时条件中心约`1.0115`，仍掉档时约`0.9808`；不是校准预测。s500结果前不追投其他scale、不启动训练。
...
- [x] 发布当前最高分I-13 `0.9978`队友复现包：`assets/derived/releases/e3_userres_r80_retkl_v3_s875/`包含I-10父训练集与I-12残差训练集的完整确定性gzip、manifest及原始小型审计；portable configs和`scripts/reproduce/i13_highscore.sh`覆盖父训练、残差训练与0.875拼接。两份解压数据均与历史训练输入逐字节一致，给定历史父/残差adapter重新拼接的r80也与线上包逐字节一致。
