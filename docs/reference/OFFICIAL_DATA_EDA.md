...
2026-07-14实验专属复核将旧的`<800字符`阈值移除后，6,460个推荐题面组中共有538组以“如/（/、/半个列表”等非终止字符结束，原始涉及1,845行，最终答案都完整；旧启发式只命中其中425组/1,495行，漏掉113个较长截断组。`seed_cotfix_v1_lora_ep1=0.8674`只测试了24–53字的确定性套话补尾与无同配方对照的r32×1ep组合，其world/action崩坏不能作为“忠实语义补全无效”的因果证据。该旧实现关闭；I-18只允许按实验专属builder、历史侧O3证据和独立质检做单变量复核。
...
- 用确定性通用套话修补截断CoT的`cotfix_v1`、暴力action token加权、focal loss、评测形态转写；均已有负证据。该条不外推到I-18的历史证据约束语义修复。
